...
public class cameraMa : MonoBehaviour
{
    ...
    public Vector3 pos;  // カメラの初期位置を記憶するための変数
    ...
    void Update()
    {
        ...
        cameraPos.x = target.transform.position.x + 5;  // カメラの横位置にキャラクターの位置+5を入れる
        
        // もし対象の縦位置が０より小さい場合
        if(target.transform.position.y < 0)
        {
            cameraPos.y = target.transform.position.y;  // カメラの縦位置を0に入れる
        }
        // もし対象の縦位置が0より大きい場合
        if(target.transform.position.y > 0)
        {
